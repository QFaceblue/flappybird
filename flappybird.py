...
class Model3(parl.Model):
    def __init__(self, act_dim):
        ...
        hid3_size = 32
        # 3层全连接网络
        self.fc1 = layers.fc(size=hid1_size, act='relu', name="f1")
        self.fc2 = layers.fc(size=hid2_size, act='relu', name="f2")
        self.fc3 = layers.fc(size=hid3_size, act='relu', name="f3")
        self.fc4 = layers.fc(size=act_dim, act=None, name="f4")
...
class Agent(parl.Agent):
    ...
    def set_e_greed(self,e_greed):
        self.e_greed = e_greed
        return self.e_greed

    def get_e_greed(self):
        return self.e_greed
...
if __name__ == "__main__":
    ...
    # # 加载模型
    # save_path = '.\model_dir\model2_700_917.6.ckpt'
    ...
    while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
        ...
        if eval_reward > evmax:
            save_path = './model_dir/model2_' + str(episode) + '_' + str(eval_reward) + '.ckpt'
            agent.save(save_path)
            evmax = eval_reward
            print("save current model with new evmax!")
    save_path = './model_dir/model2_' + str(episode) + '_' + str(eval_reward) + '.ckpt'
    agent.save(save_path)
